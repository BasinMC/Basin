 ...
 */
public interface ExtensionResolveEvent<S> extends ExtensionPhaseEvent<S> {
  ...
  final class Pre extends AbstractStatefulExtensionEvent<State> implements
      ExtensionResolveEvent<State> {

    public Pre(@Nonnull Extension extension) {
      this(extension, State.RESOLVE);
    }
  ...
  final class Post extends AbstractExtensionEvent<Void> implements ExtensionResolveEvent<Void> {
